/*
 * @fileoverview    {ConceptosRepository} se encarga de realizar tareas específicas.
 *
 * @version         2.0
 *
...
/**
 * TODO: Definición de {@code ConceptosRepository}.
 *
 * @author Dyson Parra
 * @since 1.8
 */
...
public interface ConceptosRepository extends JpaRepository<Conceptos, Long> {
     ...
     */
    public List<Conceptos> findByIntIdConcepto(Long id);
    ...
    @Query("SELECT m FROM Conceptos m WHERE m.intIdConcepto LIKE CONCAT('%', :strSearch, '%')")
    public List<Conceptos> searchEntities(@Param("strSearch") String strSearch);
    ...
    // TODO: Especificar campo de la entidad donde realizar la búsqueda.
    @Query("SELECT m FROM Conceptos m WHERE m.intIdConcepto LIKE CONCAT('%', :strSearch, '%')")
    public Page<Conceptos> searchEntities(@Param("strSearch") String strSearch, Pageable pageable);
}
