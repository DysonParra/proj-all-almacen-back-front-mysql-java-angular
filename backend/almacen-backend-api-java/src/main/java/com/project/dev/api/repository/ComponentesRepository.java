/*
 * @fileoverview    {ComponentesRepository} se encarga de realizar tareas específicas.
 *
 * @version         2.0
 *
...
/**
 * TODO: Definición de {@code ComponentesRepository}.
 *
 * @author Dyson Parra
 * @since 1.8
 */
...
public interface ComponentesRepository extends JpaRepository<Componentes, Long> {
     ...
     */
    public List<Componentes> findByIntIdComponente(Long id);
    ...
    @Query("SELECT m FROM Componentes m WHERE m.intIdComponente LIKE CONCAT('%', :strSearch, '%')")
    public List<Componentes> searchEntities(@Param("strSearch") String strSearch);
    ...
    // TODO: Especificar campo de la entidad donde realizar la búsqueda.
    @Query("SELECT m FROM Componentes m WHERE m.intIdComponente LIKE CONCAT('%', :strSearch, '%')")
    public Page<Componentes> searchEntities(@Param("strSearch") String strSearch, Pageable pageable);
}
