/*
 * @fileoverview    {TipoUnidadMedidaRepository} se encarga de realizar tareas específicas.
 *
 * @version         2.0
 *
...
public interface TipoUnidadMedidaRepository extends JpaRepository<TipoUnidadMedida, Long> {
     ...
     */
    public List<TipoUnidadMedida> findByIntIdTipoUnidadMedida(Long id);
    ...
    @Query("SELECT m FROM TipoUnidadMedida m WHERE m.intIdTipoUnidadMedida LIKE CONCAT('%', :strSearch, '%')")
    public List<TipoUnidadMedida> searchEntities(@Param("strSearch") String strSearch);
    ...
    // TODO: Especificar campo de la entidad donde realizar la búsqueda.
    @Query("SELECT m FROM TipoUnidadMedida m WHERE m.intIdTipoUnidadMedida LIKE CONCAT('%', :strSearch, '%')")
    public Page<TipoUnidadMedida> searchEntities(@Param("strSearch") String strSearch, Pageable pageable);
}
