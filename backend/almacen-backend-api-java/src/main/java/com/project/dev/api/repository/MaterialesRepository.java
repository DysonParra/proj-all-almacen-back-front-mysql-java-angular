/*
 * @fileoverview    {MaterialesRepository} se encarga de realizar tareas específicas.
 *
 * @version         2.0
 *
...
public interface MaterialesRepository extends JpaRepository<Materiales, Long> {
     ...
     */
    public List<Materiales> findByIntIdMaterial(Long id);
    ...
    @Query("SELECT m FROM Materiales m WHERE m.intIdMaterial LIKE CONCAT('%', :strSearch, '%')")
    public List<Materiales> searchEntities(@Param("strSearch") String strSearch);
    ...
    // TODO: Especificar campo de la entidad donde realizar la búsqueda.
    @Query("SELECT m FROM Materiales m WHERE m.intIdMaterial LIKE CONCAT('%', :strSearch, '%')")
    public Page<Materiales> searchEntities(@Param("strSearch") String strSearch, Pageable pageable);
}
