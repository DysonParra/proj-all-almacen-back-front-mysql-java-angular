/*
 * @fileoverview    {BodegasRepository} se encarga de realizar tareas específicas.
 *
 * @version         2.0
 *
...
/**
 * TODO: Definición de {@code BodegasRepository}.
 *
 * @author Dyson Parra
 * @since 1.8
 */
...
public interface BodegasRepository extends JpaRepository<Bodegas, Long> {
     ...
     */
    public List<Bodegas> findByIntIdBodega(Long id);
    ...
    @Query("SELECT m FROM Bodegas m WHERE m.intIdBodega LIKE CONCAT('%', :strSearch, '%')")
    public List<Bodegas> searchEntities(@Param("strSearch") String strSearch);
    ...
    // TODO: Especificar campo de la entidad donde realizar la búsqueda.
    @Query("SELECT m FROM Bodegas m WHERE m.intIdBodega LIKE CONCAT('%', :strSearch, '%')")
    public Page<Bodegas> searchEntities(@Param("strSearch") String strSearch, Pageable pageable);
}
