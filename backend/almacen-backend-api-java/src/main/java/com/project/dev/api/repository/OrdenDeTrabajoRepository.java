/*
 * @fileoverview    {OrdenDeTrabajoRepository} se encarga de realizar tareas específicas.
 *
 * @version         2.0
 *
...
/**
 * TODO: Definición de {@code OrdenDeTrabajoRepository}.
 *
 * @author Dyson Parra
 * @since 1.8
 */
...
public interface OrdenDeTrabajoRepository extends JpaRepository<OrdenDeTrabajo, Long> {
     ...
     */
    public List<OrdenDeTrabajo> findByIntIdOrdenTrabajo(Long id);
    ...
    @Query("SELECT m FROM OrdenDeTrabajo m WHERE m.intIdOrdenTrabajo LIKE CONCAT('%', :strSearch, '%')")
    public List<OrdenDeTrabajo> searchEntities(@Param("strSearch") String strSearch);
    ...
    // TODO: Especificar campo de la entidad donde realizar la búsqueda.
    @Query("SELECT m FROM OrdenDeTrabajo m WHERE m.intIdOrdenTrabajo LIKE CONCAT('%', :strSearch, '%')")
    public Page<OrdenDeTrabajo> searchEntities(@Param("strSearch") String strSearch, Pageable pageable);
}
