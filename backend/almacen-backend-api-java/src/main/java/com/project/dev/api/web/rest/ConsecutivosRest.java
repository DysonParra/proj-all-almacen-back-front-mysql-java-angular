/*
 * @fileoverview    {ConsecutivosRest} se encarga de realizar tareas específicas.
 *
 * @version         2.0
 *
...
import com.project.dev.api.web.rest.assembler.ConsecutivosRestAssembler;
...
/**
 * TODO: Definición de {@code ConsecutivosRest}.
 *
 * @author Dyson Parra
 * @since 1.8
 */
...
@Api(value = "Consecutivos API")
public class ConsecutivosRest {

    private final Logger log = LoggerFactory.getLogger(ConsecutivosRest.class);
    private final ConsecutivosService entityService;
    private final ConsecutivosRestAssembler entityRestAssembler;
     ...
     */
    public ConsecutivosRest(ConsecutivosService entityService,
            ConsecutivosRestAssembler entityRestAssembler) {
    ...
    public CollectionModel<EntityModel<ConsecutivosDTO>> getAllEntities() {
        ...
        try {
            ...
            CollectionModel<EntityModel<ConsecutivosDTO>> entitiesCollection = new CollectionModel<>(entities);
            entitiesCollection.add(linkTo(methodOn(ConsecutivosRest.class).getAllEntities()).withSelfRel());
    ...
    public ResponseEntity<List<ConsecutivosDTO>> getAllEntitiesPaged(Pageable pageable) {
    ...
    @GetMapping("/Consecutivos/{query}/pages")
    public ResponseEntity<List<ConsecutivosDTO>> searchEntitiesPaged(@PathVariable String query, Pageable pageable) {
        ...
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/Consecutivos/{query}/pages/" + query);
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
