/*
 * @fileoverview    {SociedadRepository} se encarga de realizar tareas específicas.
 *
 * @version         2.0
 *
...
/**
 * TODO: Definición de {@code SociedadRepository}.
 *
 * @author Dyson Parra
 * @since 1.8
 */
...
public interface SociedadRepository extends JpaRepository<Sociedad, Long> {
     ...
     */
    public List<Sociedad> findByIntIdSociedad(Long id);
    ...
    @Query("SELECT m FROM Sociedad m WHERE m.intIdSociedad LIKE CONCAT('%', :strSearch, '%')")
    public List<Sociedad> searchEntities(@Param("strSearch") String strSearch);
    ...
    // TODO: Especificar campo de la entidad donde realizar la búsqueda.
    @Query("SELECT m FROM Sociedad m WHERE m.intIdSociedad LIKE CONCAT('%', :strSearch, '%')")
    public Page<Sociedad> searchEntities(@Param("strSearch") String strSearch, Pageable pageable);
}
