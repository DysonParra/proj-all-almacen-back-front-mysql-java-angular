/*
 * @fileoverview    {OrdenProduccionRepository} se encarga de realizar tareas específicas.
 *
 * @version         2.0
 *
...
/**
 * TODO: Definición de {@code OrdenProduccionRepository}.
 *
 * @author Dyson Parra
 * @since 1.8
 */
...
public interface OrdenProduccionRepository extends JpaRepository<OrdenProduccion, String> {
     ...
     */
    public List<OrdenProduccion> findByStrNumeroOrden(String id);
    ...
    @Query("SELECT m FROM OrdenProduccion m WHERE m.strNumeroOrden LIKE CONCAT('%', :strSearch, '%')")
    public List<OrdenProduccion> searchEntities(@Param("strSearch") String strSearch);
    ...
    // TODO: Especificar campo de la entidad donde realizar la búsqueda.
    @Query("SELECT m FROM OrdenProduccion m WHERE m.strNumeroOrden LIKE CONCAT('%', :strSearch, '%')")
    public Page<OrdenProduccion> searchEntities(@Param("strSearch") String strSearch, Pageable pageable);
}
