/*
 * @fileoverview    {SaldosRepository} se encarga de realizar tareas específicas.
 *
 * @version         2.0
 *
...
/**
 * TODO: Definición de {@code SaldosRepository}.
 *
 * @author Dyson Parra
 * @since 1.8
 */
...
public interface SaldosRepository extends JpaRepository<Saldos, Long> {
     ...
     */
    public List<Saldos> findByIntIdSaldo(Long id);
    ...
    @Query("SELECT m FROM Saldos m WHERE m.intIdSaldo LIKE CONCAT('%', :strSearch, '%')")
    public List<Saldos> searchEntities(@Param("strSearch") String strSearch);
    ...
    // TODO: Especificar campo de la entidad donde realizar la búsqueda.
    @Query("SELECT m FROM Saldos m WHERE m.intIdSaldo LIKE CONCAT('%', :strSearch, '%')")
    public Page<Saldos> searchEntities(@Param("strSearch") String strSearch, Pageable pageable);
}
