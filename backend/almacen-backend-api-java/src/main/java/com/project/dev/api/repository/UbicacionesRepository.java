/*
 * @fileoverview    {UbicacionesRepository} se encarga de realizar tareas específicas.
 *
 * @version         2.0
 *
...
/**
 * TODO: Definición de {@code UbicacionesRepository}.
 *
 * @author Dyson Parra
 * @since 1.8
 */
...
public interface UbicacionesRepository extends JpaRepository<Ubicaciones, Long> {
     ...
     */
    public List<Ubicaciones> findByIntIdUbicacion(Long id);
    ...
    @Query("SELECT m FROM Ubicaciones m WHERE m.intIdUbicacion LIKE CONCAT('%', :strSearch, '%')")
    public List<Ubicaciones> searchEntities(@Param("strSearch") String strSearch);
    ...
    // TODO: Especificar campo de la entidad donde realizar la búsqueda.
    @Query("SELECT m FROM Ubicaciones m WHERE m.intIdUbicacion LIKE CONCAT('%', :strSearch, '%')")
    public Page<Ubicaciones> searchEntities(@Param("strSearch") String strSearch, Pageable pageable);
}
